    ...
    public partial class FrmVentas : Form
    {
      ...
      //////////////////////////////////////////////////////////////////////////////
       ...
       //////////////////////////////////////////////////////////////////////////////
        ...
        {
            if (ValidarCamposAgregar() == true  && ValidarExiste() ==true  )
            {
        ...
        void HacerDescuento()
        {
            ...
            //mostramos la suma en el textbox y en la fila que agregamos
            this.lblTotal.Text = Convert.ToString(resul) ;
        ...
        bool ValidarFinalizar() 
        {

            if (this.dataGridViewVentas.Rows.Count == 0)
            {

                MessageBox.Show("No hay Ninguna Compra Realizada");
                this.DialogResult = DialogResult.None;
                this.txtCodigo.Focus();
                return false;
            }
        ...
        private DataTable SelectFactura()
        {
            DataTable dt = new DataTable();
            // SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.AppSettings["INF420"]);
           // SqlConnection conn = new SqlConnection(@"Data Source=PC-PC;Initial Catalog=INF420;Integrated Security=True");
            ...
            SqlCommand cmd = new SqlCommand("SELECT  * FROM tblDetalles where ID_Factura= @ID_Factura", conn);
        ...
        // variables pal reporte
        string NombreComapniaReporte = string.Empty;
        string DireccionReporte = string.Empty;
        string TelefonoReporte = string.Empty;
        string IDFacturaReporte = string.Empty;
        string FechaReporte = string.Empty;
        string HoraReporte = string.Empty;
        string ClienteReporte = string.Empty;
        string CajeroReporte = string.Empty;

        string Desc = string.Empty;
        string TotalDesc = string.Empty;
        string TotalFull = string.Empty;  


        private void ImprimirReporte()
        {
            NombreComapniaReporte = Convert.ToString(this.txtNombreCompania.Text);
            DireccionReporte = Convert.ToString(this.txtDireccionCompania.Text);
            TelefonoReporte = Convert.ToString(this.txtTelefonoCompania.Text);
            IDFacturaReporte = Convert.ToString(this.txtFactura.Text);
            FechaReporte = Convert.ToString(this.txtFecha.Text);
            HoraReporte = Convert.ToString(this.txtHora.Text);
            ClienteReporte = Convert.ToString(this.txtCliente.Text);
            CajeroReporte = Convert.ToString(this.txtCajero.Text);

            Desc = Convert.ToString(this.lblTotalDescuento.Text);
            TotalDesc = Convert.ToString(this.lblTotalConDescuento.Text);
            TotalFull = Convert.ToString(this.lblTotal.Text);

            frmReporteCompra frm = new frmReporteCompra(); 
            frm.StartPosition = FormStartPosition.CenterScreen;
            ReportParameter[] paramCollection = new ReportParameter[11];

            paramCollection[0] = new ReportParameter("NombreCompania", NombreComapniaReporte, true);
            paramCollection[1] = new ReportParameter("DireccionCompania", DireccionReporte, true);
            paramCollection[2] = new ReportParameter("TelefonoCompania", TelefonoReporte, true);
            paramCollection[3] = new ReportParameter("IDFactura", IDFacturaReporte, true);
            paramCollection[4] = new ReportParameter("Fecha", FechaReporte, true);
            paramCollection[5] = new ReportParameter("Hora", HoraReporte, true);
            paramCollection[6] = new ReportParameter("Cliente", ClienteReporte, true);
            paramCollection[7] = new ReportParameter("Cajero", CajeroReporte, true);

             paramCollection[8] = new ReportParameter("desc", Desc, true);
             paramCollection[9] = new ReportParameter("totalDesc", TotalDesc, true);
             paramCollection[10] = new ReportParameter("totalFull", TotalFull, true);
           

            frm.reportViewer1.ShowPrintButton = true;

            frm.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("DataDetalle", SelectFactura().DefaultView));

            frm.reportViewer1.LocalReport.ReportEmbeddedResource = "PuntoDeVenta.ReporteCompra.rdlc";
        ...
        {
            if (ValidarFinalizar() == true && Resta > 0)
        ...
        private void txtDinero_TextChanged(object sender, EventArgs e)
        {
            ...
            this.lblTotal.Text = Convert.ToString(resul);
               ...
               // Resta = decimal.Parse(txtDinero.Text) - resul;
            Resta = decimal.Parse(txtDinero.Text) - decimal.Parse(this.lblTotalConDescuento.Text);
                lblCambio.Text = Resta.ToString();
            

            if (Resta < 0) // pa ver si es negativo
            {
                lblCambio.Text = "0.00";
            }
        }
        ...
        private void dataGridViewVentas_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
